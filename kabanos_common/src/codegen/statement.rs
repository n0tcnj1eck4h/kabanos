    ...
    'a: 'ctx,
{
    pub fn build_statement(&mut self, stmt: semantic::Statement) -> CodegenResult {
        ...
        match stmt {
            ...
            Loop(condition, body) => self.build_loop(condition, body),
            Block(Scope { variable_id, body }) => self.build_scope(variable_id, body),
            Return(expression) => self.build_return(expression),
